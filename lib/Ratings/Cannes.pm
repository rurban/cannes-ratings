...
#  }
#  # todo: set Last-Modified header based on this, the templates and the dat
#  my @files = (__FILE__, $dat);
#  no strict 'refs';
...
get '/all' => sub {
  ...
  for my $year (@YEARS) {
    ...
    if (-e $dat) {
      ...
      eval "require Ratings::$BASE\::$year;"
        or die "invalid year $year";
    }
    my $DATA = ${"Ratings::$BASE\::$year\::DATA"};
    my $HEADER = ${"Ratings::$BASE\::$year\::HEADER"};
    my $FOOTER = ${"Ratings::$BASE\::$year\::FOOTER"};
    my @critics = @{"Ratings::$BASE\::$year\::critics"};
    my @critics_group = @{"Ratings::$BASE\::$year\::critics_group"};
  ...
  {
    no strict 'refs';
    my $lastyear = $YEARS[$#YEARS];
    $all->{side_details} = _side_details(\%critic, \%title,
                             \@{"Ratings::$BASE\::$lastyear\::critics_group"});
  }
